  ...
  return (
    <div className="sarkari-wrapper">
      <div className="sarkari-card">
        ...
        <header className="sarkari-title-box" style={{ backgroundColor: post.isFromScraper ? '#000080' : '#0000FF' }}>
          <h1>{post.title}</h1>
        </header>
        ...
        <div style={{ textAlign: 'center', padding: '10px 0', borderBottom: '1px solid #ddd', fontSize: '0.9rem' }}>
          ...
          <span style={{ color: '#666' }}>{post.isFromScraper ? 'Live Updates' : 'Latest Jobs'}</span>
          <span style={{ margin: '0 8px' }}>»</span>
          <span style={{ color: '#d00', fontWeight: 'bold' }}>{post.title}</span>
        </div>

        {/* Featured Image (Only for Sanity Posts) */}
        {!post.isFromScraper && post.mainImage && (
        ...
        {/* Short Information Section (Scraped content often has this inside the HTML already) */}
        {!post.isFromScraper && post.body && (
        ...
        {/* Main Content Area (The Tables) */}
        <div className="sarkari-content-area" style={{ padding: post.isFromScraper ? '0' : '5px' }}>
          {post.htmlBody && (
            <div 
              className={post.isFromScraper ? 'scraped-tables' : ''}
      ...
      {/* Background info for Scraped content (Optional indicator) */}
      {post.isFromScraper && (
